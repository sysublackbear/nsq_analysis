...
func (c *Conn) Connect() (*IdentifyResponse, error) {
	...
	if resp != nil && resp.AuthRequired {  // 需要认真
	...
	c.wg.Add(2)
	atomic.StoreInt32(&c.readLoopRunning, 1)  // 打开loop开关
	go c.readLoop()
	go c.writeLoop()
	return resp, nil
}
...
func (c *Conn) Close() error {
	...
	if c.conn != nil && atomic.LoadInt64(&c.messagesInFlight) == 0 {
		// todo: 这里为什么要CloseRead?
		return c.conn.CloseRead()  // 没有在发的消息，关闭读？
	}
	return nil
}
...
func (c *Conn) SetRDY(rdy int64) {
	...
	if rdy > 0 {
		atomic.StoreInt64(&c.lastRdyTimestamp, time.Now().UnixNano())  // 修改laste_update_time
	}
}
...
func (c *Conn) readLoop() {
	...
	for {
		...
		if frameType == FrameTypeResponse && bytes.Equal(data, []byte("_heartbeat_")) {
			...
			c.delegate.OnHeartbeat(c)  // 心跳的回调函数
