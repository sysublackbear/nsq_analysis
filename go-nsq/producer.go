...
type Producer struct {
	...
	transactions    []*ProducerTransaction  // 有点搞不明白这结构体是用来做什么的
	state           int32
...
// sencCommand -> router -> conn.WriteCommand
...
func (w *Producer) connect() error {
	...
	_, err := w.conn.Connect()  // conn.go中的Connect
	if err != nil {
		w.conn.Close()
