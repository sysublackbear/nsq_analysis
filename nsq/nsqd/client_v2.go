...
func newClientV2(id int64, conn net.Conn, ctx *context) *clientV2 {
	...
	if conn != nil {
		identifier, _, _ = net.SplitHostPort(conn.RemoteAddr().String())  // 划分出IP和Port
	}
...
func (c *clientV2) Identify(data identifyDataV2) error {
	...
	// 设置心跳时间间隔（设置HeartbeatInterval的值）
	err := c.SetHeartbeatInterval(data.HeartbeatInterval)
	if err != nil {
		return err
	}

	// 初始化bufio.Writer(设置Writer，OutputBufferTimeout, OutputBufferSize)
	...
	select {
	case c.IdentifyEventChan <- ie:  // 塞入IdentifyEventChan
	default:
	}
...
func (c *clientV2) IsProducer() bool {
	c.metaLock.RLock()
	retval := len(c.pubCounts) > 0  // todo:根据这个来进行判断
	c.metaLock.RUnlock()
	return retval
}
...
func (c *clientV2) tryUpdateReadyState() {
	...
	select {
	case c.ReadyStateChan <- 1:  // 塞入状态准备好的Channel
	default:
	}
}

func (c *clientV2) FinishedMessage() {
	atomic.AddUint64(&c.FinishCount, 1)  // FinishCount++
	atomic.AddInt64(&c.InFlightCount, -1)  // InFlightCount--
	c.tryUpdateReadyState()
}

func (c *clientV2) Empty() {
	atomic.StoreInt64(&c.InFlightCount, 0)  // 将InFlightCount置为0
	c.tryUpdateReadyState()
}

func (c *clientV2) SendingMessage() {
	atomic.AddInt64(&c.InFlightCount, 1)  // InFlightCount++
	atomic.AddUint64(&c.MessageCount, 1)  // MessageCount++
}

func (c *clientV2) PublishedMessage(topic string, count uint64) {
	c.metaLock.Lock()
	c.pubCounts[topic] += count  // 广播消息数topic, +count
	c.metaLock.Unlock()
}

func (c *clientV2) TimedOutMessage() {
	atomic.AddInt64(&c.InFlightCount, -1)  // 消息超时,InFlightCount--
	c.tryUpdateReadyState()
}

func (c *clientV2) RequeuedMessage() {
	atomic.AddUint64(&c.RequeueCount, 1)
	atomic.AddInt64(&c.InFlightCount, -1)  // 重入队消息
	c.tryUpdateReadyState()
}

func (c *clientV2) StartClose() {
	// Force the client into ready 0
	c.SetReadyCount(0)  // 设置无可用的客户端
	// mark this client as closing
	atomic.StoreInt32(&c.State, stateClosing)
}

// todo: 为什么Pause和UnPause都调用的是tryUpdateReadyState?
...
func (c *clientV2) SetHeartbeatInterval(desiredInterval int) error {
	...
	case desiredInterval >= 1000 &&
		...
		c.HeartbeatInterval = time.Duration(desiredInterval) * time.Millisecond // 单位为秒
	default:
...
func (c *clientV2) UpgradeTLS() error {
	...
	err := tlsConn.Handshake()  // 类似https握手
	if err != nil {
		return err
	}
	c.tlsConn = tlsConn
...
func (c *clientV2) Auth(secret string) error {
	c.AuthSecret = secret
	return c.QueryAuthd()  // 调用auth的http接口
}
...
func (c *clientV2) IsAuthorized(topic, channel string) (bool, error) {
	...
	// 根据返回的登录态，判断topic和channel是否具有权限
