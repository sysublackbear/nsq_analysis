...
函数runtime.SetBlockProfileRate的唯一参数是int类型的。它的含义是分析器会在每发生几次Goroutine阻塞事件时对这些事件进行取样。
如果我们不显式的使用runtime.SetBlockProfileRate函数设置取样间隔，那么取样间隔就为1。
也就是说，在默认情况下，每发生一次Goroutine阻塞事件，分析器就会取样一次。
与内存使用情况记录一样，运行时系统对Goroutine阻塞事件的取样操作也会贯穿于用户程序的整个运行期。
但是，如果我们通过runtime.SetBlockProfileRate函数将这个取样间隔设置为0或者负数，那么这个取样操作就会被取消。
 */
...
func (s *httpServer) pingHandler(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	health := s.ctx.nsqd.GetHealth()  // 判断nsqd服务是否健康
...
func (s *httpServer) getTopicFromQuery(req *http.Request) (url.Values, *Topic, error) {
	...
	// 和GetExistingTopic的区别：不存在则创建topic
...
func (s *httpServer) doPauseTopic(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	s.ctx.nsqd.Lock()
	s.ctx.nsqd.PersistMetadata()  // todo:这里要看清楚为啥要这样
	s.ctx.nsqd.Unlock()
	return nil, nil
}
...
// 修改或查询NSQD的配置信息(PUT方法修改，GET方法查询）
